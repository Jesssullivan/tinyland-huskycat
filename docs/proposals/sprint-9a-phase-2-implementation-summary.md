...
⚠️  Auto-DevOps validation passed with 2 warning(s)
```
...
1. **Template Rendering Errors**:
   ...
   ```
   ⚠️ Only caught by `helm template` (skipped in fast mode)
...
2. **K8s API Compatibility**:
   ...
   kind: Deployment
   ```
   ⚠️ Only caught by `kubectl --dry-run` (skipped in fast mode)
